...
public class Client {
    ...
    private static String processCommand(String query) {
        ...
        if (((command.name).equals("login")) || ((command.name).equals("register")) || (username != null)) {
            ...
            switch (command.name) {
                ...
                case "remove":
                case "removegreater": {
                    return doWithHatArgument1(command.name, command.argument);
                }
                case "multiline": {
                    multiline = !multiline;
        ...
        else {return "вы не авторизированны";}
    }
    /**
    ...
    private static String doWithHatArgument(String command, String jsonArgument) {
        try {
            if (jsonArgument == null)
                ...
                String[] p = jsonArgument.split(" ",2);
                Hat[] hats;
                try {
                    if (p[1] == null)
                    hats = HatMaker.generate(jsonArgument);
                    else {
                        hats = HatMaker.generate(p[1]);
                    }
                } catch (Exception e) {

                    return "Не удалось создать шляпу. Введите \"help add\", чтобы увидеть правильный формат. " + e.getMessage() ;
                }
                try (SocketChannel channel = SocketChannel.open()) {
                    ...
                    for (int i = 0; i < hats.length; i++) {
                        ...
                        Message message = new Message<>(command, p[0], hats[i]);
                        message.setUserName(Client.username);
                        message.setPassword(Client.password);
    ...
    private static String doWithHatArgument1(String command, String jsonArgument) {
        try {
            if (jsonArgument == null)
                return sendCommand(command, null);
            else {
                ...
                try (SocketChannel channel = SocketChannel.open()) {
                    ...
                    for (int i = 0; i < hats.length; i++) {
                        // Making a Message instance and writing it to ByteArrayOutputStream
                        Message message = new Message<>(command,  hats[i]  );
                        message.setUserName(Client.username);
                        message.setPassword(Client.password);
                        oos.writeObject(message);
                    }
        ...
        try (SocketChannel channel = SocketChannel.open()) {
            ...
            while (true) {
                ...
                try {
                    ...
                    Client.password = argumentpass[1];
    ...
    private static String doShow() {
        try (SocketChannel channel = SocketChannel.open()) {
          ...
          if (!incoming.hasArgument()) System.out.println("Ничего не передано");

          if (incoming.getArgument() instanceof String)
              a = (String) incoming.getArgument();

          else System.out.println("Сервер вернул данные в неверном формате");

            // Writing the response to System.out
          if (a.length() > 0) return "Шляпы в гардеробе: " + " \n" + a;
          else return "Гардероб пуст";
